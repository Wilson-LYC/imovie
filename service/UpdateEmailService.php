...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $code=$_POST['code'];
    //判断邮箱是否已经注册，已注册报错
    $res=$userDao->selectUserByEmail($email);
    if(count($res)>0){
        $msg="该邮箱已注册";
        $link=VIEW_PATH.'updateEmail.php';
        $linkText="重试";
    }else{
        //判断验证码是否正确
        if($code==$_SESSION['code']){
            //更新数据库
            $res=$userDao->updateUserByUidEmail($_SESSION['user']['uid'],$email);
            if($res){
    ...
    $link=VIEW_PATH.'updateEmail.php';
    $linkText='返回';
    header('Location:'.VIEW_PATH.'updateEmail.php');
}
?>
<!DOCTYPE html>
<head>
